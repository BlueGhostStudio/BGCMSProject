...
EditorWindow::EditorWindow(CMSApi *api, QWidget *parent)
  ...
  QObject::connect(ui_contentEditor, &QsciScintilla::modificationChanged, this,
                   [=]() {
...
void EditorWindow::load(const QVariantMap &_node, const QString &type) {
  CallGraph::start("getNode", this)
      ...
      ->nodes(
          ...
          [=](CallGraph* cg, const QVariant &data) {
            ...
            ui_contentEditor->SendScintilla(
                QsciScintilla::SCI_STYLESETFONT, QsciScintilla::STYLE_BRACEBAD,
                m_defaultFont.family().toLatin1().data());
            ...
            if (settings
              ...
              ui_contentEditor->setLexer(&m_lexerHTML);
                         ...
                         .value("contentEditor/syntax/md", QVariantList({"md"}))
                         .toList()
                         .contains(type))
              ui_contentEditor->setLexer(&m_lexerMarkdown);
            else if (settings
...
void EditorWindow::initialEditor() {
...
void EditorWindow::setSyntaxHighlight(const QString &type) {}
...
void EditorWindow::save(CallGraph* pcg, const QString &to,
                        const QString &error) {
...
void EditorWindow::findBackwards() {
  if (ui_contentEditor->hasSelectedText())
    ui_contentEditor->SendScintilla(
        QsciScintilla::SCI_GOTOPOS,
        ui_contentEditor->SendScintilla(QsciScintilla::SCI_GETSELECTIONSTART));
